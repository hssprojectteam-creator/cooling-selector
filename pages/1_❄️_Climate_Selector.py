...
# Correct back-relative routing to find the folder from the pages subfolder
SLIDES_DIR = os.path.abspath(os.path.join(BASE_DIR, "..", "slides"))
...
def load_cooling_data():
    excel_path = os.path.abspath(os.path.join(BASE_DIR, "..", "CoolingData.xlsx"))
    try:
        df = pd.read_excel(excel_path, sheet_name="CoolingData")
    except Exception:
...
try:
    ...
    logo_path = os.path.abspath(os.path.join(BASE_DIR, "..", "logo.png"))
    if os.path.exists(logo_path):
        st.sidebar.image(logo_path, width="stretch" if hasattr(st.sidebar, "image") else None)
    else:
